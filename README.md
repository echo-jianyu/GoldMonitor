...
- 💰 **五大行情模块实时同步**：**伦敦现货黄金 (XAU)**、**上金所 Au99.99**、**黄金延期 Au(T+D)**，以及 **京东金融民生 / 浙商积存金**，实时报价与涨跌幅计算。
...
- 🎨 **高度个性化定制**：
  ...
  - 5 个行情模块均支持独立开关、独立标签文字与配色，统一显示规则一键同步。
...
- **模块显示**：5 个行情模块独立启用/停用、自定义标签文字与颜色，统一显示规则一键套用（默认仅显示国际金价与浙商积存金）。
