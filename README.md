...
<p align="center">
  ...
  <img src="docs/preview_translucent.png" alt="静默潜伏效果" width="480" /><br/>
...
- 💰 **六大行情模块实时同步**：**伦敦现货黄金 (XAU)**、**上金所 Au99.99**、**黄金延期 Au(T+D)**、**XAU→CNY 换算金价**，以及 **京东金融民生 / 浙商积存金**，实时报价与涨跌幅计算。
- 🍃 **极致绿色单文件**：仅一个独立 `.exe`（约 2MB），**无需安装任何 .NET 运行时**，在纯净 Windows 10 / 11 电脑上双击秒开。
- 🖥️ **圆角胶囊美学设计**：深色拟态极简胶囊，支持矢量等比无级缩放（0.5x ~ 3.0x），在高分辨率 2K/4K 屏上依然细腻清晰。
...
- 🎨 **高度个性化定制**：
  ...
  - 6 个行情模块均支持独立开关、独立标签文字与配色，统一显示规则一键同步。
...
### 下载运行
1. 前往 **[Releases 页面](https://github.com/echo-jianyu/GoldMonitor/releases)** 下载最新版 `GoldMonitor.exe`；
...
- **外观透明度**：静默与鼠标悬停时的透明度（5%~100% 整数档位）、全局 UI 缩放比例微调。
- **模块显示**：6 个行情模块独立启用/停用、自定义标签文字与颜色，统一显示规则一键套用（默认仅显示国际金价与浙商积存金）。
...
1. 本软件为开源个人桌面小工具，行情数据来源于新浪财经与京东金融等公开网络接口。
